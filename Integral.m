...
% Observation: n must be multiply of 4 [4 80 120 260 400 800 2000]
...
% Create a array of spaces X values
XX = linspace(upperLimit, lowerLimit, 3/4*n + 1);

% The size of each step
h = XX(1,4) - XX(1,1);

% Calculate the Sum of the small areas (F*h)
fun = 0;
for k = 1:3: (3/4*n)
   fun = fun + (h/8) * (subs(F,XX(1,k)) + 3*(subs(F,XX(1,k+1)) + subs(F,XX(1,k+2))) + subs(F,XX(1,k+3)));
end


%% OUTPUT SECTION
% Print the output in Command window
sprintf('Numerical Integration - Method of  Newton Cortes (3/8 Simpson)\nF = %s      Bounds = [ %.f ; %.f ]\nNº points = %.f      delta(H) = %.3e \n\nArea = %s ', F, lowerLimit, upperLimit ,n, h, vpa(fun))

% clearvars x F upperLimit lowerLimit n XX h
